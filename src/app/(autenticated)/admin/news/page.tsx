...
const AdminNewsPage = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setFormData({ id: '', title: '', summary: '', image: '', date: '' });
      setImageFile(null);
      fetchNews();
    } catch (err: any) {
      setError(err.message);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/news/${itemToDeleteId}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      <div className="flex items-center gap-4 animate-slideUp">
        ...
        <h1 className="text-3xl md:text-4xl font-extrabold">Manajemen Berita</h1>
      </div>

      {/* Formulir Tambah/Edit Berita */}
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 transition-all hover:shadow-lg animate-fadeIn delay-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <input
            ...
            required
            className="w-full p-3 rounded-md border border-gray-300 dark:bg-gray-700 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
          />
          <textarea
            name="summary"
          ...
          <div className="flex space-x-2">
            ...
            >
              {loading ? 'Menyimpan...' : formData.id ? 'Simpan Perubahan' : 'Tambah Berita'}
            </button>
            {formData.id && (
              <button
                type="button"
                onClick={() => setFormData({ id: '', title: '', summary: '', image: '', date: '' })}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 transition-all hover:shadow-lg animate-fadeIn delay-200">
        <h2 className="text-xl font-semibold mb-4">Daftar Berita ({news.length})</h2>
